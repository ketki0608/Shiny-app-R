      ...
      tabsetPanel(
        ...
        tabPanel("Risk Offset", plotOutput("plotRisk"))
             ...
             tabPanel(
               ...
               tags$ol(
                 tags$li(
                   "Packages: ggplot, scales, quadprog, quantreg, shiny, qrmdata, xts, matrixStats, zoo, QRM, plotly, and psych"
                 ),
                 tags$li(
                   "Used shiny to build web application to create visually appealing data story, plots and commentaries. 
                   We split app file into subsections like ui, server and business logic to make it manageable and maintainable."
                 ),
                 tags$li(
                 ...
                 tags$li("Utilized the Markowitz model and the Global Minimum Variance (GVM) model to define the optimal mix of commodities within the portfolio by running simulations using both models. The GVM model was chosen because it sets a mix of assets in order to minimize volatility in lieu of building a model to maximize returns (the optimal portfolio). It tends to outperform a classic optimized portfolio."),
                 tags$li("We additional learnt about Sharpe’s curve and the tangent portfolio, even though parts to the left of it are impossible to have that risk and return, but besides that we learned about how to translate that info from words to application.")
               ),
             ...
             tabPanel("Personal Contribution",
                      tags$ol(tags$li("Designed and built the shiny app. Our earlier code was structured to work with flex dashboard, but flexdashboard did not offer much flexibility in terms of representation of data.
                                      So we decided to go fot shiny app. I restructured entire code to be function based and split it in layers- UI, 
                                      server and business logic. This made the code more manageable and easy to update."),
                              tags$li("Added Data exploratory analysis."),
                              tags$li("Added Market Analysis for stocks and currencies."),
                              tags$li("Added Extreme tail analysis- Mean Excess Loss, Pareto distribution fit and extreme tail probabilities for different confidence intervals"))),
